  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (error) {
        ...
        return errorResponse('Erro ao listar pacotes de festa.', 500);
      }
    ...
    if (method === 'POST') {
      ...
      if (!isValidPositiveInteger(price_cents)) {
        return errorResponse(
          'O campo price_cents é obrigatório e deve ser um número inteiro positivo representando o valor em centavos (ex: 29900 para $299.00).',
          400
        );
      }
      ...
      if (isFeaturedHome) {
        ...
        if (!countErr && typeof currentFeaturedCount === 'number' && currentFeaturedCount >= 3) {
          return errorResponse(
            'Você já tem 3 pacotes de festa destacados. Desmarque um antes de adicionar outro.',
            400
          );
        }
      }
      ...
      const insertData = {
        ...
        guest_count: guest_count ? Number(guest_count) : 10,
        items_included: Array.isArray(items_included) ? items_included : [],
      ...
      if (insertError) {
        ...
        return errorResponse(`Erro ao criar pacote de festa: ${insertError.message || 'Erro no banco'}`, 500);
      }
    ...
    if (method === 'PUT') {
      ...
      if (price_cents !== undefined) {
        if (!isValidPositiveInteger(price_cents)) {
          return errorResponse(
            'O campo price_cents deve ser um número inteiro positivo representando o valor em centavos.',
            400
          );
        }
      ...
      if (duration_minutes !== undefined) {
        updateData.duration_minutes = Number(duration_minutes) || 120;
      }

      if (guest_count !== undefined) {
        updateData.guest_count = Number(guest_count) || 10;
      }

      if (items_included !== undefined) {
        updateData.items_included = Array.isArray(items_included) ? items_included : [];
      }
      ...
      if (updateError) {
        ...
        return errorResponse(`Erro ao atualizar pacote de festa: ${updateError.message || 'Erro no banco'}`, 500);
      }

      if (!updatedPackage) {
        return errorResponse('Pacote de festa não encontrado.', 404);
      }
